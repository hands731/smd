...
def kill_process(process_path):
    try:
        print(f"Attempting to kill process: {process_path}")
        # kill_process.sh 실행 전에 프로세스가 실행 중인지 확인
        check_process = subprocess.check_output(f"ps aux | grep {process_path}", shell=True, text=True)
        print(f"Current processes: {check_process}")
        
        result = subprocess.check_output(['/home/pi/Current/kill_process.sh', process_path], text=True)
        print(f"Kill process result: {result}")
        
        # kill 후 프로세스 상태 다시 확인
        check_after = subprocess.check_output(f"ps aux | grep {process_path}", shell=True, text=True)
        print(f"Processes after kill: {check_after}")
        return result
    except subprocess.CalledProcessError as e:
        print(f"Error killing process: {e.output}")
        return None
...
async def handle_messages(websocket):
    async for message in websocket:
        ...
        if data.get('status') == 'url_change':
            ...
            seq = data.get('device_id')
            browser_id = data.get('browser_id')
            returncode, stdout, stderr = await execute_command(command)
        ...
        elif data.get('status') == 'send_cnc_file':
            file_name = data.get('file_name')
            device_id = data.get('device_id')
            command = f"python3 /home/pi/Serial/sendCNCFile.py {file_name} &"
            subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            print("send success")

        elif data.get('status') == 'receive_cnc_file':
            device_id = data.get('device_id')
            try:
                print("Starting receive_cnc_file process")
        # 기존 프로세스 확인 및 종료
                check_process = subprocess.check_output("ps aux | grep receiveCNCFile.py", shell=True, text=True)
                print(f"Before kill - Current processes: {check_process}")
        
                kill_result = kill_process('/home/pi/Serial/receiveCNCFile.py')
                print(f"Kill process completed with result: {kill_result}")
        
                await asyncio.sleep(1)
        
        # 새로운 수신 프로세스를 백그라운드로 시작
                print("Starting new receiveCNCFile.py process")
                command = "python3 /home/pi/Serial/receiveCNCFile.py &"  # 백그라운드로 실행
                subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                print("Process started in background")
        
            except Exception as e:
                print(f"Error in receive_cnc_file handler: {e}")
