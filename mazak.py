...
def getMyDict(xml_data):
    ...
    for device_stream in root.findall('m:Streams/m:DeviceStream', namespaces):
        ...
        # Controller 컴포넌트에서 EmergencyStop 찾기
        controller = device_stream.find('.//m:ComponentStream[@component="Controller"]', namespaces)
        if controller is not None:
            estop = controller.find('.//m:EmergencyStop', namespaces)
            if estop is not None:
                data_dict['Estop'] = estop.text

        # Path 컴포넌트에서 나머지 데이터 찾기
        path = device_stream.find('.//m:ComponentStream[@component="Path"]', namespaces)
        if path is not None:
            # Execution
            execution = path.find('.//m:Execution', namespaces)
            if execution is not None:
                data_dict['execution'] = execution.text

            # ControllerMode
            mode = path.find('.//m:ControllerMode', namespaces)
            if mode is not None:
                data_dict['mode'] = mode.text

            # PartCount
            part_count = path.find('.//m:PartCount', namespaces)
            if part_count is not None:
                data_dict['partCount'] = part_count.text

            # ToolNumber
            tool_id = path.find('.//m:ToolNumber', namespaces)
            if tool_id is not None:
                data_dict['toolID'] = tool_id.text

            # Program
            program = path.find('.//m:Program', namespaces)
            if program is not None:
                data_dict['program'] = program.text
    ...
    for d in dict_list:
        ...
        print("Extracted data:", result)  # 추출된 데이터 출력
