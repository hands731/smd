...
ser = None
TIMEOUT_DURATION = 300  # 5분 = 300초

# 마지막 읽기 성공 시간을 저장할 변수
last_read_time = time.time()

def check_timeout():
    global last_read_time
    current_time = time.time()
    if current_time - last_read_time > TIMEOUT_DURATION:
        print(f"{TIMEOUT_DURATION}초 동안 데이터 수신이 없어 프로그램을 종료합니다.")
...
def notify_file_completion(device, filename):
    try:
        url = "http://dev-ycheon.tomes.co.kr/tomes/fileReadFinish"
        data = {
            "equip_name": device,
            "file_name": filename
        }
...
def notify_file_start(device, filename):
    try:
        url = "http://dev-ycheon.tomes.co.kr/tomes/fileReadStart"
        data = {
            "equip_name": device,
            "file_name": filename
        }
...
def read_with_retry(ser, max_retries=3):
    ...
    for _ in range(max_retries):
        try:
            check_timeout()  # 타임아웃 체크
            res = ser.readline()
            if res:
                last_read_time = time.time()  # 성공적으로 읽었을 때 시간 갱신
                return res
            time.sleep(0.1)
        except Exception as e:
